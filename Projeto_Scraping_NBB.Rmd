...
Nesse projeto, fiz uma raspagem de dados do site da NBB (Novo Basquete Brasil) para obter informações sobre as equipes e suas estatísticas. O objetivo é extrair uma tabela de classificação da NBB.
Usei a biblioteca rvest para extrair os dados da tabela diretamente do site e, em seguida, manipulei os dados para reorganizá-los conforme as instruções fornecidas. E por último, montei um relatório em R Markdown para exibir a tabela de classificação da NBB, e com isso tenho um relatório atualizado sempre que o código for executado.
Esse é apenas um exemplo de como o R pode ser usado para extrair e manipular dados da web e gerar relatórios personalizados.
...
## Exposição da classificação da NBB com um gráfico
...
# Converter a coluna PON para numérica
NBB_table$PON <- as.numeric(NBB_table$PON)

# Criar o gráfico de barras com classificação decrescente e rótulos centralizados
ggplot(NBB_table %>% arrange(desc(PON)), aes(x = reorder(EQUIPES, PON), y = PON)) +
  geom_bar(stat = "identity", fill = "darkorange") +
  geom_text(aes(label = PON), vjust = 0.5, hjust = 1.5, color = "darkgreen", size = 3.5) +
  coord_flip() +
  labs(title = "Pontuação", x = "Equipes", y = "Pontos") +
  theme_minimal() +
